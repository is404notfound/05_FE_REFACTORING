...
export default {
  ...
  setup(props, context) {
    ...
    const { dispatch: manufactureDispatch } = useGenericStore('manufacture', 'mapping');
    ...
    return {
      ...
      async onClickSearch() {
        const { srchMfId, mfIdType } = paramInfo.value;
        if (isNaN(srchMfId)) {
          const inputName: string = this.mfIdTypeUIList.find(v=> v.value === mfIdType).label;
          alert({ title: '경고', message: `${inputName}는 숫자만 입력해 주세요.` });
          return;
        }
        root.$events.$emit('LOADING_START');
        await manufactureDispatch('getMappingSearch', { ...getParamInfo(paramInfo.value) });
        emit('update:page', 1);
